...
export function StudentDashboardPage() {
  ...
  return <div className="dashboard-view student-dashboard">
    ...
    {loading ? <div className="content-loading"><Spinner />Chargement…</div> : <>
      ...
      <section className="app-panel student-next-step"><span>{accepted ? <BadgeCheck /> : <GraduationCap />}</span><div><h2>{accepted ? 'Votre université vous a confirmé' : current ? 'Votre demande est en cours' : 'Choisissez votre université'}</h2><p>{accepted ? 'Vous pouvez compléter votre profil et présenter vos compétences.' : current ? 'Vous serez notifié dès que le gestionnaire aura répondu.' : 'Envoyez une demande avec votre filière et le matricule renseigné à l’inscription.'}</p></div><a className="button" href={accepted ? '/espace-etudiant/profil' : '/espace-etudiant/affiliation'}>{accepted ? 'Compléter mon profil' : 'Gérer ma demande'}</a></section>
    </>}
  </div>;
}
...
export function StudentAffiliationPage() {
  ...
  useEffect(() => {
    apiRequest('/profils/moi', { token }).then((response) => {
      setForm((current) => ({ ...current, matriculeEtudiant: response.donnees?.matricule_etudiant || '' }));
    }).catch(() => null);
  }, [token]);
  ...
  return <div><DashboardPageHeader title="Mon affiliation" description="Demandez à votre université de confirmer votre statut étudiant." />
    ...
    <div className="student-affiliation-layout">
      <form className="app-panel affiliation-form" onSubmit={submit}><h2>Nouvelle demande</h2><p>Une seule demande peut être active à la fois.</p>
        ...
        <label className="editor-field"><span>Matricule étudiant *</span><input required minLength="2" disabled={active} value={form.matriculeEtudiant} onChange={(e) => setForm({ ...form, matriculeEtudiant: e.target.value })} /></label>
...
export function StudentProfilePage() {
  const { token } = useAuth(); const [profile, setProfile] = useState(undefined); const [form, setForm] = useState({ matriculeEtudiant: '', titreProfil: '', competences: '', anneeDiplomation: '', estVisible: true }); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [saving, setSaving] = useState(false);
  useEffect(() => { apiRequest('/profils/moi', { token }).then((r) => { const p = r.donnees; setProfile(p); setForm({ matriculeEtudiant: p.matricule_etudiant || '', titreProfil: p.titre_profil || '', competences: Array.isArray(p.competences) ? p.competences.join(', ') : '', anneeDiplomation: p.annee_diplomation || '', estVisible: Boolean(p.est_visible) }); }).catch((err) => { if (err.status === 404) setProfile(null); else setError(err.message); }); }, [token]);
