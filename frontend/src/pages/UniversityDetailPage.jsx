...
export function UniversityDetailPage() {
  ...
  const [partners, setPartners] = useState([]); const [enrollmentOpen, setEnrollmentOpen] = useState(false); const [admissionsOpen, setAdmissionsOpen] = useState(false);
  ...
  useEffect(() => {
    Promise.all([
      ...
      const fiche = institution.donnees;
      const aujourdHui = new Date().toISOString().slice(0, 10);
      const calendrierOuvert = (!fiche.date_debut_inscription || String(fiche.date_debut_inscription).slice(0, 10) <= aujourdHui)
        && (!fiche.date_fin_inscription || String(fiche.date_fin_inscription).slice(0, 10) >= aujourdHui);
      setPartners(partnerResponse.donnees || []);
      const inscriptionsActives = Boolean(fiche.inscriptions_ouvertes) && calendrierOuvert;
      setAdmissionsOpen(inscriptionsActives);
      setEnrollmentOpen(Boolean(enrollmentResponse.donnees) && inscriptionsActives);
