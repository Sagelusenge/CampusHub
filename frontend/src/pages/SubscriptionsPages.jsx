...
export function AdminSubscriptionsPage() {
  ...
  const trials = subscriptions.filter((item) => item.type_abonnement === 'ESSAI' && item.statut === 'ACTIF');
  return <div><DashboardPageHeader title="Abonnements et paiements" description="Suivez les essais gratuits et validez l’abonnement annuel des établissements." actions={<button className="secondary-action" onClick={load}><RefreshCw />Actualiser</button>} />
    ...
    <div className="subscription-metrics"><article><DollarSign /><div><small>Abonnement annuel</small><strong>10 $ / an</strong></div></article><article><CalendarCheck /><div><small>Essais gratuits actifs</small><strong>{trials.length}</strong></div></article><article><CreditCard /><div><small>Paiements à vérifier</small><strong>{payments.filter((x) => x.statut === 'EN_ATTENTE').length}</strong></div></article><article><CalendarClock /><div><small>Échéances à 7 jours</small><strong>{expiring.length}</strong></div></article></div>
    ...
    <section className="app-panel subscription-section"><div className="app-panel__heading"><div><h2>Accès des établissements</h2><p>Essais gratuits, abonnements annuels et comptes à rebours issus de la base de données.</p></div></div><div className="subscription-grid">{subscriptions.map((item) => <article className={item.jours_restants <= 7 ? 'subscription-card subscription-card--warning' : 'subscription-card'} key={item.code_abonnement}><div><span><CalendarCheck /></span><StatusBadge status={item.statut} /></div><h3>{item.nom_universite || item.nom_affichage}</h3><p>{item.type_abonnement === 'ESSAI' ? 'Essai gratuit de 30 jours' : 'Abonnement annuel'} • {item.email}</p><div className="countdown"><strong>{item.jours_restants}</strong><span>jour(s) restant(s)</span></div><small>Échéance : {new Date(item.date_fin).toLocaleDateString('fr-FR')}</small></article>)}</div></section>
  </div>;
}
...
export function InstitutionSubscriptionPage() {
  ...
  return <div><DashboardPageHeader title="Abonnement annuel" description="Profitez d’abord de 30 jours gratuits, puis continuez pour 10 $ par an." />{message && <div className="alert alert--success"><Check />{message}</div>}{error && <div className="alert alert--error">{error}</div>}
    <section className="current-subscription current-subscription--annual app-panel"><div><small>Accès actuel</small><h2>{subscription?.type_abonnement === 'ESSAI' ? 'Essai gratuit CampusHub' : subscription?.nom_plan || 'Aucun accès actif'}</h2><p>{subscription?.date_fin ? `${subscription.type_abonnement === 'ESSAI' ? 'Essai valable' : 'Abonnement valable'} jusqu’au ${new Date(subscription.date_fin).toLocaleDateString('fr-FR')}` : 'Votre premier compte institutionnel bénéficie de 30 jours gratuits.'}</p></div><div className="subscription-count subscription-count--light"><strong>{subscription?.jours_restants || 0}</strong><span>jours restants</span></div></section>
