...
function DynamicField({ field, value, setValue, token }) {
  const common = { id: field.id, required: field.obligatoire, value, onChange: (event) => setValue(event.target.value) };
  if (field.type === 'DOCUMENT' || field.type === 'IMAGE') return <div className="field"><span>{field.label}{field.obligatoire && ' *'}</span><FileUploadField label={field.type === 'DOCUMENT' ? 'Joindre le document demandé' : 'Joindre l’image demandée'} value={value} token={token} document={field.type === 'DOCUMENT'} onUploaded={setValue} />{field.obligatoire && !value && <small>Ce fichier est obligatoire avant l’envoi.</small>}</div>;
  return <label className="field"><span>{field.label}{field.obligatoire && ' *'}</span>{field.type === 'ZONE_TEXTE' ? <textarea {...common} rows="4" /> : field.type === 'SELECT' ? <select {...common}><option value="">Choisir…</option>{field.options.map((option) => <option key={option}>{option}</option>)}</select> : <input {...common} type={{ EMAIL: 'email', TELEPHONE: 'tel', DATE: 'date', NOMBRE: 'number' }[field.type] || 'text'} />}</label>;
}
