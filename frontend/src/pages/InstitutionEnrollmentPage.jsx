      ...
      <form className="app-panel enrollment-builder" onSubmit={save}>
        ...
        <div className="builder-fields">
          ...
          <div className="builder-fields__list">{form.champs.map((field, index) => <div className="builder-field" key={field.id}>
            ...
            <label className="builder-field__control"><span>Type de réponse</span><select value={field.type} onChange={(event) => updateField(index, 'type', event.target.value)}>{['TEXTE','EMAIL','TELEPHONE','DATE','NOMBRE','ZONE_TEXTE','SELECT','DOCUMENT','IMAGE'].map((type) => <option key={type}>{type}</option>)}</select></label>
