...
export function InstitutionDashboardPage() {
  ...
  return <div className="dashboard-view">
    ...
    <div className={`verification-strip ${abonnement?.statut === 'ACTIF' && (abonnement.est_a_vie || abonnement.jours_restants > 7) ? 'verification-strip--success' : ''}`}><span>{abonnement?.statut === 'ACTIF' ? <BadgeCheck /> : <Clock3 />}</span><div><strong>{abonnement?.statut === 'ACTIF' ? (abonnement.est_a_vie ? 'Accès CampusHub à vie actif' : `${abonnement.jours_restants} jour(s) d’abonnement restant(s)`) : 'Abonnement à renouveler'}</strong><p>{abonnement?.statut === 'ACTIF' ? (abonnement.est_a_vie ? 'Aucun renouvellement annuel n’est nécessaire pour cet établissement.' : 'Une alerte renforcée apparaîtra durant les 7 derniers jours.') : 'Choisissez l’accès annuel ou l’accès à vie pour conserver tous les services.'}</p></div><Link to="/espace-universite/abonnement">Gérer</Link></div>
