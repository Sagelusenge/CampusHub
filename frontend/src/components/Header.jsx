...
export function Header() {
  ...
  return <header className="site-header"><div className="container header-inner">
    ...
    <nav className={`main-nav ${menuOpen ? 'main-nav--open' : ''}`} aria-label="Navigation principale">
      <NavLink to="/" end onClick={() => setMenuOpen(false)}>Accueil</NavLink><NavLink to="/universites" onClick={() => setMenuOpen(false)}>Universités</NavLink><NavLink to="/reseau" onClick={() => setMenuOpen(false)}>Réseau</NavLink><NavLink to="/contact" onClick={() => setMenuOpen(false)}>Contact</NavLink>
    ...
    <div className="header-actions"><button className="icon-button header-search" aria-label="Rechercher" onClick={() => navigate('/reseau')}><Search size={19} /></button>
      {estConnecte ? <><Link className="button button--small" to={espace}>{utilisateur?.role === 'VISITEUR' ? 'Mon réseau' : 'Mon espace'}</Link><button className="icon-button" aria-label="Se déconnecter" onClick={quitter}><LogOut size={19} /></button></> : <><Link className="header-signup" to="/inscription-visiteur">Créer un compte</Link><Link className="button button--small" to="/connexion">Connexion</Link></>}
